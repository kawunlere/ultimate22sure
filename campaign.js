...
if (!campaign) {
  ...
  if (!sessionStorage.getItem(visitorKey)) {
    ...
    let updated = JSON.parse(localStorage.getItem('u22_campaigns')) || [];
    updated = updated.map(c => {
      if (c.id === campaign.id) c.visitors = (c.visitors || 0) + 1;
      return c;
    });
    localStorage.setItem('u22_campaigns', JSON.stringify(updated));
  }
...
function updateProgress() {
  ...
  if (percent === 100 && unlockFinal) {
    ...
    if (!sessionStorage.getItem(completedKey)) {
      ...
      let updated = JSON.parse(localStorage.getItem('u22_campaigns')) || [];
      updated = updated.map(c => {
        if (c.id === campaign.id) c.completed = (c.completed || 0) + 1;
        return c;
      });
      localStorage.setItem('u22_campaigns', JSON.stringify(updated));
    }
